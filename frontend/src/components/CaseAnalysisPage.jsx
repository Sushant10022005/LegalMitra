...
const CaseAnalysisPage = () => {
  ...
  const analyzeDocument = async () => {
    ...
    try {
      ...
      const responsePromise = axios.post(`${API_BASE_URL}/api/analyze-document`, formData, {
        headers: {
          'Content-Type': 'multipart/form-data',
        },
        onUploadProgress: (progressEvent) => {
          const percentCompleted = Math.round((progressEvent.loaded * 100) / progressEvent.total);
          setProcessingProgress(30 + (percentCompleted * 0.3)); // 30-60% for upload
        },
      });
      ...
      if (err.message === 'Request timed out') {
        setError('The request took too long to process. Please try again with a smaller file or different document.');
      } else {
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-lg shadow-xl max-w-md w-full">
            <div className="flex flex-col items-center">
              ...
              <p className="text-gray-700 font-medium mb-2">{processingStatus}</p>
              <p className="text-sm text-gray-500">
                {processingProgress < 30 ? 'Preparing...' :
                 processingProgress < 60 ? 'Uploading...' :
                 processingProgress < 90 ? 'Analyzing...' :
                 'Processing results...'}
              </p>
            </div>
          </div>
